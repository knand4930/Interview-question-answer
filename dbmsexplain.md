...
Geographic and Spatial Queries (PostGIS)
Advanced Security
...
Usage Example:SELECT MAX(salary) AS second_highest_salary
FROM employees 
...
Usage Example:SELECT DISTINCT salary 
FROM employees 
...
Usage Example:SELECT name, salary
FROM employees e
...
Usage Example:SELECT d.department_name, COUNT(e.employee_id)
FROM departments d
...
Usage Example:HAVING COUNT(e.employee_id) > 5;
...
Usage Example:SELECT name, hire_date
FROM employees
...
Usage Example:SELECT email, COUNT(*) AS count
FROM employees
GROUP BY email
HAVING COUNT(*) > 1;
...
Usage Example:SELECT name, department_id, salary,
...
Usage Example:SELECT sale_date, amount,
...
Usage Example:SELECT employee_id, salary,
...
Usage Example:SELECT order_date, amount,
...
Usage Example:WITH monthly_sales AS (
...
Usage Example:WITH RECURSIVE employee_hierarchy AS (
...
Usage Example:SELECT DISTINCT e1.name, e2.name, e1.salary
FROM employees e1
...
Usage Example:SELECT COALESCE(pending.pending_orders, 0) AS pending_orders
FROM products p
...
Usage Example:SELECT product_name
FROM products p
...
Usage Example:SELECT name, EXTRACT(YEAR FROM AGE(CURRENT_DATE, birth_date)) AS age
FROM employees;
...
Usage Example:SELECT generate_series(order_date, delivery_date, '1 day'::interval)
FROM orders;
...
Usage Example:SELECT email, SUBSTRING(email FROM '@(.*)$') AS domain
FROM employees;
...
Usage Example:SELECT email, SPLIT_PART(email, '@', 2) AS domain
FROM employees;
...
Usage Example:SELECT REGEXP_REPLACE(phone, '[^0-9]', '', 'g') AS clean_phone
FROM employees;
...
Usage Example:SELECT data->>'name' AS name
FROM user_profiles
...
Usage Example:SELECT data->'address'->>'city' AS city
FROM user_profiles;
...
Usage Example:SELECT JSON_ARRAY_LENGTH(skills) AS skill_count
...
Usage Example:SELECT JSON_ARRAY_ELEMENTS_TEXT(skills) AS individual_skills
FROM employee_skills;
...
Usage Example:UPDATE user_profiles 
...
Usage Example:SELECT 'PostgreSQL' = ANY(skills) AS knows_postgresql
FROM employees;
...
Usage Example:SELECT ARRAY_LENGTH(skills, 1) AS skill_count
FROM employees;
...
Usage Example:SELECT UNNEST(skills) AS individual_skill
FROM employees;
...
Usage Example:EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)
...
Usage Example:CREATE INDEX idx_employees_high_salary 
ON employees(salary) 
WHERE salary > 100000;
...
Usage Example:CREATE INDEX idx_employees_dept_salary 
...
Usage Example:SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY salary) AS median_salary
FROM employees
...
Usage Example:SELECT MODE() WITHIN GROUP (ORDER BY job_title) AS most_common_job
FROM employees
GROUP BY department_id;
...
Usage Example:SELECT STRING_AGG(name, ', ' ORDER BY name) AS employee_list
FROM employees
GROUP BY department_id;
...
Usage Example:SELECT COUNT(*) FILTER (WHERE salary > 50000) AS high_earners
FROM employees
GROUP BY department_id;
...
Usage Example:UPDATE employees
...
Usage Example:INSERT INTO employee_stats (employee_id, login_count)
VALUES (1, 1)
...
Usage Example:DELETE FROM customers c
...
Usage Example:COPY employees(name, email) FROM '/path/to/employees.csv' WITH (FORMAT CSV, HEADER true);
...
Usage Example:CREATE TYPE status_type AS ENUM ('active', 'inactive', 'pending');
...
Usage Example:SELECT * FROM bookings
...
Usage Example:ALTER TABLE employees
...
Usage Example:CREATE FUNCTION calculate_bonus(emp_id INTEGER)
RETURNS DECIMAL AS $$
DECLARE
    emp_salary DECIMAL;
BEGIN
...
Usage Example:CREATE PROCEDURE promote_employee(emp_id INTEGER, new_title VARCHAR)
LANGUAGE plpgsql AS $$
BEGIN
...
Usage Example:CREATE TRIGGER employee_audit_trigger
...
Usage Example:IF TG_OP = 'DELETE' THEN
...
Usage Example:INSERT INTO employee_audit(old_values) VALUES (row_to_json(OLD));
...
Usage Example:CREATE TABLE sales_data (id SERIAL, sale_date DATE) PARTITION BY RANGE (sale_date);
...
Usage Example:CREATE TABLE sales_2023 PARTITION OF sales_data
...
Usage Example:ALTER TABLE articles ADD COLUMN search_vector tsvector;
...
Usage Example:UPDATE articles SET search_vector = to_tsvector('english', title || ' ' || content);
...
Usage Example:SELECT title FROM articles, to_tsquery('english', 'postgresql & performance') query
...
Usage Example:SELECT title, ts_rank(search_vector, query) AS rank
...
Usage Example:CREATE INDEX idx_articles_search ON articles USING GIN(search_vector);
...
Usage Example:SELECT FIRST_VALUE(customers) OVER (PARTITION BY cohort_month ORDER BY period_number)
FROM cohort_table;
...
Usage Example:SELECT NTILE(5) OVER (ORDER BY recency_days ASC) AS recency_score
...
Usage Example:SELECT name, email
FROM employees
...
Usage Example:SELECT ARRAY_AGG(employee_id) AS employee_ids
FROM employees
GROUP BY email
HAVING COUNT(*) > 1;
...
Usage Example:SELECT e.name, ph.total_hours
FROM employees e
...
Usage Example:CREATE EXTENSION IF NOT EXISTS postgis;
...
Usage Example:SELECT store_name, ST_Distance(location, ST_MakePoint(-122.4194, 37.7749)) AS distance
FROM stores;
...
Usage Example:SELECT store_name
FROM stores
...
Usage Example:ALTER TABLE employee_data ENABLE ROW LEVEL SECURITY;
...
Usage Example:CREATE POLICY employee_data_policy ON employee_data
...
Usage Example:SELECT department_id FROM user_departments WHERE username = current_user;
...
Usage Example:SELECT query, total_time
...
Usage Example:SELECT blocked_locks.pid, blocked_activity.query
...
Usage Example:SELECT pg_size_pretty(pg_database_size(current_database())) AS db_size;
...
Usage Example:SELECT employee_id, 'Q1' AS quarter, q1_sales AS sales FROM quarterly_sales
UNION ALL
...
Usage Example:CREATE MATERIALIZED VIEW monthly_sales_summary AS
...
Usage Example:REFRESH MATERIALIZED VIEW monthly_sales_summary;
...
Usage Example:SET max_parallel_workers_per_gather = 4;
...
Usage Example:CREATE FUNCTION safe_divide(a NUMERIC, b NUMERIC)
RETURNS NUMERIC AS $$
BEGIN
    RETURN a / b;
...
Usage Example:CREATE EXTENSION "uuid-ossp";
...
Usage Example:SELECT XPATH('//name/text()', xml_data)[1]::TEXT AS name
FROM user_profiles;
...
Usage Example:SELECT client_ip
FROM network_logs
...
Usage Example:BEGIN ISOLATION LEVEL REPEATABLE READ;
...
Usage Example:SELECT pg_try_advisory_lock(12345);
...
Usage Example:SELECT indexname, idx_scan
...
Usage Example:SELECT ROUND(100.0 * heap_blks_hit / (heap_blks_hit + heap_blks_read)) AS cache_hit_ratio
...
Usage Example:SELECT pg_create_restore_point('before_data_modification');
...
Usage Example:CREATE PUBLICATION my_publication FOR TABLE employees;
...
Usage Example:SELECT CORR(years_experience, salary) AS correlation
FROM employees;
...
Usage Example:SELECT REGR_SLOPE(salary, years_experience) AS slope
FROM employees;
...
Usage Example:SELECT region_name, product_category, SUM(amount)
FROM sales
...
Usage Example:SELECT GROUPING(region_name, product_category) AS grouping_level
FROM sales
GROUP BY CUBE(region_name, product_category);
...
Usage Example:CREATE FUNCTION update_customer_scd(p_customer_id INTEGER, p_name VARCHAR)
RETURNS VOID AS $$
BEGIN
...
Usage Example:SELECT ARRAY[employee_id] AS path
FROM employees;
...
Usage Example:SELECT d.date, p.product_name, SUM(fs.revenue)
FROM fact_sales fs
...
Usage Example:CREATE EXTENSION pgcrypto;
...
Usage Example:CREATE FUNCTION mask_credit_card(cc_number TEXT) RETURNS TEXT
...
Usage Example:CREATE FUNCTION validate_employee_data()
...
Usage Example:SELECT STDDEV_POP(salary) AS population_stddev
FROM employees;
...
Usage Example:SELECT COUNT(*) FROM orders
...
Usage Example:SELECT state, COUNT(*) FROM pg_stat_activity
GROUP BY state;
...
Usage Example:SELECT relname, seq_scan
...
Usage Example:SELECT CEILING(avg_daily_sales * 7) AS reorder_point
FROM inventory;
...
Usage Example:SELECT CEILING(SQRT(2 * avg_daily_sales * order_cost / carrying_cost)) AS eoq
FROM inventory;
...
Usage Example:SELECT ROW_NUMBER() OVER (PARTITION BY year ORDER BY revenue DESC) AS revenue_rank
FROM sales_cube;
...
Usage Example:SELECT total_revenue / NULLIF(order_count, 0) AS avg_order_value
FROM sales;
